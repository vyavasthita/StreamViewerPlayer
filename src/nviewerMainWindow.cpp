...
NViewerMainWindow::NViewerMainWindow(QWidget *parent) :
    QWidget(parent)
  , mIsReady(false)
...
NViewerMainWindow::~NViewerMainWindow()
{
    if (m_preview) {
        m_preview->close();
        delete m_preview;
...
void NViewerMainWindow::initPlayer()
{
...
void NViewerMainWindow::onSeekFinished(qint64 pos)
{
    qDebug("seek finished at %lld/%lld", pos, mpPlayer->position());
}

void NViewerMainWindow::stopUnload()
{
    mpPlayer->stop();
}

void NViewerMainWindow::setupUi()
{
...
void NViewerMainWindow::changeChannel(QAction *action)
{
...
void NViewerMainWindow::changeAudioTrack(QAction *action)
{
...
void NViewerMainWindow::changeVO(QAction *action)
{
...
void NViewerMainWindow::processPendingActions()
{
    if (mHasPendingPlay) {
        mHasPendingPlay = false;
        play(mFile);
...
void NViewerMainWindow::setAudioBackends(const QStringList& backends)
{
    mAudioBackends = backends;
}

bool NViewerMainWindow::setRenderer(QtAV::VideoRenderer *renderer)
{
    if (!renderer)
        return false;
    if (!renderer->widget()) {
...
void NViewerMainWindow::play(const QString &name)
{
    mFile = name;
    if (!mIsReady) {
...
void NViewerMainWindow::play(const QUrl &url)
{
    play(QUrl::fromPercentEncoding(url.toEncoded()));
}

void NViewerMainWindow::setVideoDecoderNames(const QStringList &vd)
{
    QStringList vdnames;
...
void NViewerMainWindow::openFile()
{
...
void NViewerMainWindow::togglePlayPause()
{
...
void NViewerMainWindow::showNextOSD()
{
    if (!mpOSD)
        return;
...
void NViewerMainWindow::onSpinBoxChanged(double v)
{
    if (!mpPlayer)
        return;
    mpPlayer->setSpeed(v);
...
void NViewerMainWindow::onPaused(bool p)
{
    if (p) {
...
void NViewerMainWindow::onStartPlay()
{
...
void NViewerMainWindow::onStopPlay()
{
    mpPlayer->setPriority(idsFromNames(Config::instance().decoderPriorityNames()));
...
void NViewerMainWindow::onSpeedChange(qreal speed)
{
    mpSpeed->setText(QString::fromLatin1("%1").arg(speed, 4, 'f', 2, QLatin1Char('0')));
}

void NViewerMainWindow::setFrameRate()
{
    if (!mpPlayer)
        return;
    mpPlayer->setFrameRate(Config::instance().forceFrameRate());
...
void NViewerMainWindow::seek(int value)
{
...
void NViewerMainWindow::seek()
{
    seek(mpTimeSlider->value());
}

void NViewerMainWindow::showHideVolumeBar()
{
...
void NViewerMainWindow::setVolume()
{
...
void NViewerMainWindow::closeEvent(QCloseEvent *e)
{
    Q_UNUSED(e);
    if (mpPlayer)
        mpPlayer->stop();
...
void NViewerMainWindow::resizeEvent(QResizeEvent *e)
{
    Q_UNUSED(e);
...
void NViewerMainWindow::timerEvent(QTimerEvent *e)
{
...
void NViewerMainWindow::onPositionChange(qint64 pos)
{
...
void NViewerMainWindow::repeatAChanged(const QTime& t)
{
    if (!mpPlayer)
        return;
...
void NViewerMainWindow::repeatBChanged(const QTime& t)
{
    if (!mpPlayer)
        return;
...
void NViewerMainWindow::keyPressEvent(QKeyEvent *e)
{
    mControlOn = e->key() == Qt::Key_Control;
}

void NViewerMainWindow::keyReleaseEvent(QKeyEvent *e)
{
    Q_UNUSED(e);
    mControlOn = false;
}

void NViewerMainWindow::mousePressEvent(QMouseEvent *e)
{
    if (!mControlOn)
        return;
...
void NViewerMainWindow::mouseMoveEvent(QMouseEvent *e)
{
    unsetCursor();
...
void NViewerMainWindow::wheelEvent(QWheelEvent *e)
{
    if (!mpRenderer || !mpRenderer->widget()) {
        return;
    }
...
void NViewerMainWindow::openUrl()
{
...
void NViewerMainWindow::updateChannelMenu()
{
    if (mpChannelAction)
        mpChannelAction->setChecked(false);
    AudioOutput *ao = mpPlayer ? mpPlayer->audio() : 0; //getAO()?
...
void NViewerMainWindow::initAudioTrackMenu()
{
    int track = -2;
    QAction *a = 0;
    QList<QAction*> as;
...
void NViewerMainWindow::switchAspectRatio(QAction *action)
{
...
void NViewerMainWindow::toggleRepeat(bool r)
{
...
void NViewerMainWindow::setRepeateMax(int m)
{
    mRepeateMax = m;
    if (mpPlayer) {
...
void NViewerMainWindow::playOnlineVideo(QAction *action)
{
    mTitle = action->text();
    play(action->data().toString());
}

void NViewerMainWindow::onTVMenuClick()
{
...
void NViewerMainWindow::onPlayListClick(const QString &key, const QString &value)
{
    mTitle = key;
    play(value);
}

void NViewerMainWindow::tryHideControlBar()
{
    if (mShowControl > 0) {
        return;
    }
...
void NViewerMainWindow::tryShowControlBar()
{
    unsetCursor();
...
void NViewerMainWindow::showInfo()
{
    if (!mpStatisticsView)
...
void NViewerMainWindow::onTimeSliderHover(int pos, int value)
{
...
void NViewerMainWindow::onTimeSliderLeave()
{
...
void NViewerMainWindow::handleError(const AVError &e)
{
    QMessageBox::warning(0, tr("Player error"), e.string());
}

void NViewerMainWindow::onMediaStatusChanged()
{
    QString status;
...
void NViewerMainWindow::onBufferProgress(qreal percent)
{
...
void NViewerMainWindow::onVideoEQEngineChanged()
{
...
void NViewerMainWindow::onBrightnessChanged(int b)
{
    VideoRenderer *vo = mpPlayer->renderer();
...
void NViewerMainWindow::onContrastChanged(int c)
{
    VideoRenderer *vo = mpPlayer->renderer();
    if (mpVideoEQ->engine() != VideoEQConfigPage::SWScale
...
void NViewerMainWindow::onHueChanged(int h)
{
    VideoRenderer *vo = mpPlayer->renderer();
    if (mpVideoEQ->engine() != VideoEQConfigPage::SWScale
...
void NViewerMainWindow::onSaturationChanged(int s)
{
    VideoRenderer *vo = mpPlayer->renderer();
    if (mpVideoEQ->engine() != VideoEQConfigPage::SWScale
...
void NViewerMainWindow::onCaptureConfigChanged()
{
...
void NViewerMainWindow::onAVFilterVideoConfigChanged()
{
    if (mpVideoFilter) {
...
void NViewerMainWindow::onAVFilterAudioConfigChanged()
{
    if (mpAudioFilter) {
...
void NViewerMainWindow::onAbortOnTimeoutChanged()
{
    if (!mpPlayer)
        return;
    mpPlayer->setInterruptOnTimeout(Config::instance().abortOnTimeout());
...
void NViewerMainWindow::onUserShaderChanged()
{
...
void NViewerMainWindow::setup()
{
    ConfigDialog::display();
}

void NViewerMainWindow::handleFullscreenChange()
{
    workaroundRendererSize();
...
void NViewerMainWindow::toggoleSubtitleEnabled(bool value)
{
    mpSubtitle->setEnabled(value);
}

void NViewerMainWindow::toggleSubtitleAutoLoad(bool value)
{
    mpSubtitle->setAutoLoad(value);
}

void NViewerMainWindow::openSubtitle()
{
...
void NViewerMainWindow::setSubtitleCharset(const QString &charSet)
{
    Q_UNUSED(charSet);
...
void NViewerMainWindow::setSubtitleEngine(const QString &value)
{
    Q_UNUSED(value)
    QComboBox *box = qobject_cast<QComboBox*>(sender());
    if (!box)
...
void NViewerMainWindow::changeClockType(QAction *action)
{
    action->setChecked(true);
...
void NViewerMainWindow::syncVolumeUi(qreal value)
{
...
void NViewerMainWindow::workaroundRendererSize()
{
    if (!mpRenderer)
        return;
    QSize s = rect().size();
